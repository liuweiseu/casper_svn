...
map_type = get_var('map_type', 'defaults', defaults, varargin{:});
...
reuse_block(blk, 'pre_sync_delay', 'xbsIndex_r3/Delay', ...
...
reuse_block(blk, 'post_sync_delay', 'xbsIndex_r3/Delay', ...
    'Position', [135   159   155   201], 'latency', num2str(bram_latency+1+double_buffer));
reuse_block(blk, 'delay_valid', 'xbsIndex_r3/Delay', ...
    'Position', [495    13    525    27], 'latency', num2str(bram_latency+1+double_buffer));
reuse_block(blk, 'delay_we', 'xbsIndex_r3/Delay', ...
...
if order == 1,
    ...
    for i=1:n_inputs,
        ...
        % Delays
        reuse_block(blk, ['delay_din',num2str(i-1)], 'xbsIndex_r3/Delay', ...
...
elseif double_buffer == 0,   
    ...
    for i=1:n_inputs,
        ...
        reuse_block(blk, ['delay_din',num2str(i-1)], 'xbsIndex_r3/Delay', ...
            'Position', [550    80*i    590    80*i+20], 'latency', num2str((order-1)*map_latency+1));
        reuse_block(blk, ['bram',num2str(i-1)], 'xbsIndex_r3/Single Port RAM', ...
    ...
    for i=1:order-1,
        ...
        if map_type == 0,
            try
                if ~strcmp(get_param([blk,'/',mapname], 'MaskType'), 'Xilinx Single Port Read-Only Memory'),
                    delete_block([blk,'/',mapname]);
                end
            catch
                disp('deleted nonrom')
            end
            reuse_block(blk, mapname, 'xbsIndex_r3/ROM', ...
                ...
                'distributed_mem', 'on', 'Position', [230  125+50*i   270    145+50*i]);
        else,
            try
                if ~strcmp(get_param([blk,'/',mapname], 'MaskType'), 'map'),
                    delete_block([blk,'/',mapname]);
                end
            catch
                disp('deleted nonmap')
            end
            reuse_block(blk, mapname, 'casper_library/FFTs/map', ...
                'map', mat2str(map), 'latency', num2str(map_latency), ...
                'Position', [230  125+50*i   270    145+50*i]);
        end
        reuse_block(blk, ['delay_',mapname], 'xbsIndex_r3/Delay', ...
...
% case for order > 1, double-buffered
    ...
    if map_type == 0,
        ...
        reuse_block(blk, mapname, 'xbsIndex_r3/ROM', ...
            'depth', num2str(map_length), 'initVector', mat2str(map), 'latency', num2str(map_latency), ...
            'arith_type', 'Unsigned', 'n_bits', num2str(map_bits), 'bin_pt', '0', ...
            'distributed_mem', 'on', 'Position', [230  125+50   270    145+50]);
    else,
        try
            if ~strcmp(get_param([blk,'/',mapname], 'MaskType'), 'map'),
                delete_block([blk,'/',mapname]);
        ...
        reuse_block(blk, mapname, 'casper_library/FFTs/map', ...
            'map', mat2str(map), 'latency', num2str(map_latency), ...
            'Position', [230  125+50   270    145+50]);
    end
