...
if strcmp(s.hw_sys, 'iBOB') & ~isempty(strmatch('usr_clk', s.clk_src))
    if ~isempty(strmatch(s.gpioclk_grp, {'zdok0', 'zdok1', 'mdr'}));
        ext_ports.usrclk_in_p = {1 'in' 'usrclk_in_p' [s.gpioclk_hw_sys, '.', s.gpioclk_grp,'_p([',num2str(s.gpioclkbit),']+1)'] 'LVDS_25_DT' 'vector=false'};
        ext_ports.usrclk_in_n = {1 'in' 'usrclk_in_n' [s.gpioclk_hw_sys, '.', s.gpioclk_grp,'_n([',num2str(s.gpioclkbit),']+1)'] 'LVDS_25_DT' 'vector=false'};
    else
