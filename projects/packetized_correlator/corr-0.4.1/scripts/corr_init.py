...
if __name__ == '__main__':
    ...
    p.add_option('-r', '--n_retries', dest='n_retries', type='int', default=20, 
        help='Number of times to try after an error before giving up. Default: 20')
...
try:
    ...
    for x in range(c.config['n_xaui_ports_per_fpga']):
        c.write_int_all('base_ant%i'%x,((2**32)-1))
    ...
    for x in range(c.config['n_xaui_ports_per_fpga']):
        for f in range(c.config['n_ants']/c.config['n_ants_per_xaui']/c.config['n_xaui_ports_per_fpga']):
            c.fpgas[f].write_int('base_ant%i'%x, ant)
            print('%i'%ant),
            sys.stdout.flush()
