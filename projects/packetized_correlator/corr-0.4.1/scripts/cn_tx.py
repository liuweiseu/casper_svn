...
import time, os, threading, subprocess, socket, struct, sys
...
class CorrTX:
    ...
    def empty_buffers(self):
        ...
        for xnum in range(self.x_per_fpga):
            print '\t: Flushed %i vectors for X engine %i. %i/%i complete.'%(total_xeng_vectors[xnum], xnum, sum(complete),self.x_per_fpga)
    ...
    def _tx(self):
        ...
        while not self._quit:
            ...
            for xnum in range(n_xeng):
                print '[%6i] Grabbed %i vectors for X engine %i with timestamp %i.'%(n_integrations,int_xeng_vectors[xnum], xnum, rounded_timestamp[xnum])
                data[xnum] = ''.join(data[xnum])
                for cnt in range((len(data[xnum])/self.payload_len)):
                    if self.casper_sock.send_packet(rounded_timestamp[xnum], xnum, cnt*self.payload_len, 139264, data[xnum][cnt*self.payload_len:(cnt+1)*self.payload_len]) != target_pkt_size: print 'TX fail!' 
                data[xnum]=[]
                complete[xnum]=0
