...
'''
import matplotlib, time, corr, numpy, struct, sys, logging, pylab
...
def getUnpackedData(requiredPol):
    ...
    return real(unpacked_vals), imag(unpacked_vals), requiredFpga
...
try:
    ...
    def drawDataCallback(comparePlots):
        ...
        maxY = 0
        for pol in polList:
            unpackedreal,unpackedimag, ffpga = getUnpackedData(pol)
            matplotlib.pyplot.subplot(len(polList), 1, counter)
            histData = pylab.histogram(unpackedreal, range(-1,1))[0]
            ...
            histData = pylab.histogram(unpackedimag, range(-1,1))[0]
            maxY = max(maxY, max(histData))
            matplotlib.pyplot.bar(range(-1,1), histData)
            if not comparePlots:
                matplotlib.pyplot.ylim(ymax = max(histData))
