...
if __name__ == '__main__':
    ...
    p = OptionParser()
    p.set_usage('snap_gbe_tx.py [options] CONFIG_FILE')
        ...
        for i in range(len(bram_dmp[brams[1]][f])/4):
            ...
            if bram_oob[f]['eof'][i] and not opts.raw:
                ...
                if pkt_len != packet_len+1:
                    ...
                    #print mcnts
                    print 'HDR @ %4i. MCNT %12u. Freq: %4i. Ant: %3i. 4 bit power: PolQ: %4.2f, PolI: %4.2f'%(hdr_index,feng_unpkd_pkt['pkt_mcnt'],feng_unpkd_pkt['pkt_freq'],feng_unpkd_pkt['pkt_ant'],feng_unpkd_pkt['rms_polQ'],feng_unpkd_pkt['rms_polI'])
