...
//static void recvudp_cmd(int, char**);
static void bramfill_cmd(int, char**);
...
//static tinysh_cmd_t cmd_21 = {0, "recvudp", "","",recvudp_cmd,0,0,0};
static tinysh_cmd_t cmd_21 = {0, "bramfill", "Fills LUT BRAM", "", bramfill_cmd,0,0,0};

/* Globals for the udp server */
/* ------------------------------- */
struct udp_pcb *udppcb, *recvpcb;
struct ip_addr addr, addr_loc;
struct pbuf *somebuf;
int i=0, j=0, k=0, b=0;
int numpackets;
int waittime;
...
static void bramfill_cmd(int argc, char **argv){
    	int i = 0;
	...
	bram2write = XPAR_IBOB_BRAM_SHARED_BRAM_BASEADDR;

	if(argc == 1){
		for(i = 0; i < 14; i++)
			XIo_Out32(bram2write + i*4, bp_value[i]);
	}
	else{
		for(i = 0; i < 14; i++)
			XIo_Out32(bram2write + i*4, bp_value[tinysh_atoxi(argv[1])]);
	}
