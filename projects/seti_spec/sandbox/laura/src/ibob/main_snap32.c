...
display_welcome_msg(void)
{
  ...
  xil_printf("\n\r");
  xil_printf("**********************************\n\r");
  xil_printf("* TinySH lightweight shell w UDP *\n\r");
  xil_printf("**********************************\n\r");
...
dumpbram = XPAR_IBOB_BRAM_SNAP_BRAM_BASEADDR;
  ...
  while(1) 
  {
	...
	{  			
		for(j = 0; j < 8; j++) //256 x 8 = 2048 channels = 8192 bytes
		{
			...
			{
				bootpacket[0] = (char) j;
				dirxvalue = XIo_In32(dumpbram + (i*4) + (j*1024)); //BRAM addressed by byte
