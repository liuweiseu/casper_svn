/*********************************/
/* Modified by Laura - 11 Nov 08 */
/* Reading out 64-bit snap BRAM  */
...
display_welcome_msg(void)
{
  ...
  xil_printf("*     SNAP64 over UDP      *\n\r");     
  xil_printf("****************************\n\r");

#define DESIGN_NAME "ibob_bram"
#define COMPILED_ON "Nov-2008"
...
static void startudp_cmd(int argc, char **argv)
{
	...
	udpport = htons(tinysh_atoxi(argv[5]));
...
bram2send[0] = XPAR_IBOB_BRAM_SNAP64_BRAM_LSB_BASEADDR;
bram2send[1] = XPAR_IBOB_BRAM_SNAP64_BRAM_MSB_BASEADDR;
	  ...
	  loadindicator = 9999;
	  udpswitch = 1;
	  accnum = sif_reg_read(XPAR_IBOB_BRAM_SNAP64_ADDR_BASEADDR);  //Get snap address
}
...
xil_printf("lwip_init\n");
  ...
  while(1) 
  {
	...
	{  			
		for(k = 0; k < 2; k++)
		{
			for(j = 0; j < 8; j++) //256 x 8 = 2048 channels = 8192 bytes
			{
				for(i = 0; i < 256; i++) //~1024 bytes allowed = 256 channels
				{
					bootpacket[0] = (char) j;
					dirxvalue = XIo_In32(bram2send[k] + (i*4) + (j*1024)); //BRAM addressed by byte
					memcpy(bootpacket + 4 + (i*4), &dirxvalue, 4);
				}
