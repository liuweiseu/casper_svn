...
typedef struct {
...
#endif
#if FALSE
		7apr99 removed .. added filler
		double  xAoMs;		/* no longer valid*/
		double  xVtxMs;     /* no longer valid*/
#endif
		} AGC_STAT_POS_ERRS;
	...
	<pjp030> this block
typedef struct {
	int		grSecMid;		/* seconds midnite last greg measure*/
	int		grDat;			/* last data byte from greg*/
	int		chSecMid;		/* seconds midnite last greg measure*/
	int		chDat;			/* last data byte from ch*/
	} AGC_STAT_OVRSPD;
#endif
/*
...
typedef struct {
		...
		VTX_ERR_LIST gen;		/* generic errors 3 int long*/ 
		VTX_ERR_LIST az;		/* azimuth errors*/
		VTX_ERR_LIST gr;		/* gregorian errors*/
		VTX_ERR_LIST ch;		/* carriage house errors*/
		} AGC_STAT_ERRS;
typedef struct {
...
#if FALSE 
		AGC_STAT_OVRSPD ovrSpd; /* 23jun99, filled in via msg <pjp030> */
#endif 
