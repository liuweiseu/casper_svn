...
void byteswap(u_char *buf, int size) ;
void shortswap(u_char *buf, int size) ;
void intswap(u_char *buf, int size) ;
void endswap(u_char *buf, u_int type, int size) ;
void unpackrgb(u_char *buf, int size) ;
...
unpackrgb(u_char *buf, int size)
{
	...
	printf("size %x from %x buf %x tmpp %x\n",
		size, (u_int)from_p, (u_int)buf, (u_int)tmpp) ;
    /* DEBUG */
    while (from_p >= buf)
    {
	if (((u_int)tmpp % 4) == 0)
