...
typedef struct {
	...
	u_int freqs[MAXFREQ]; //TODO: change to gaps[MAXFREQ].
...
/*
 * chkgap_usage: displays the description for each command line option
 */
void chkgap_usage();


/* parse_options: get user options from command line.
 * returns: 0=success, 2=help needed */
int chkgap_parse_options(int argc, char **argv, CheckGapData *d); 

/* initialize: open and init board and program state */
void chkgap_initialize (CheckGapData *d);

/* run_test: perform the actual test */
