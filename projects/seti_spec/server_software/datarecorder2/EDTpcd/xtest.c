...
/* #pragma ident "@(#) xtest.c 1.39@(#) Copyright EDT" */
...
static u_char  last_stat;
static u_short *buf_p;
static u_int   savebuf;
static u_int   tmpbuf;
...
usage()
{
    ...
    puts("usage: xtest [-d filename] [-u unit number] [-i] [-s] size");
...
#endif
{
    ...
    {
#ifdef _NT_
	sprintf(cmdstr, ".\\bitload -u %s%d %s %s xtest.bit",
			    devname, unit, verbose? "-v":"", dir_arg);
#else
	sprintf(cmdstr, "./bitload -u %s%d %s %s xtest.bit",
			    devname, unit, verbose? "-v":"", dir_arg);
#endif
	...
	if (ret > 0)
	{
		...
		if (buf_p[i] != chkval)
		{
		    ...
		    {
			printf("word %d (byte 0x%x) was %x s/b %x\n",
			       i, i, buf_p[i], chkval);
		    }
		    if (errcnt == 10)
		    {
