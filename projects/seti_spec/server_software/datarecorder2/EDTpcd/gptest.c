/* #pragma ident "@(#)gptest.c	1.1 10/29/01 EDT" */
...
#define	USAGE "usage: %s pcd_unit_to_be_test_driver unit_under_test_pcd [-l loop]\n"
...
char *argv[];
{
    ...
    td_no = atoi(argv[0]) ;
    (void) sprintf(cmd,	"./bitload  -u %d gptest.bit", td_no);
    system(cmd)	;
    
    uut_no = atoi(argv[1]) ;
    (void) sprintf(cmd,	"./bitload  -u %d gp_pcd.bit", uut_no);
    system(cmd)	;
    ...
    for(loop = 0 ; loop < loops_todo || loops_todo == 0 ; loop++)
    {
		...
		printf("SETUP: test driver is pgp%d;  uut is pgp%d\n\n", td_no, uut_no);
		...
		*/
		 ...
		 */
		edt_intfc_write_short(edt_td, PCDT_CMD, PCDTEST_ENCHK | use_pcd_src) ;
...
/*
 * test	the function outputs of the unit under	test
...
stat_test(EdtDev *edt_uut, EdtDev *edt_td)
{
	...
	for(i=0; i < 4; i++)
	{
	    ...
	    {
			printf(" stat bit test failed was %x should be %x readback %x\n",
				was, shdbe, rdbk);
			errcnt++;
	    }
		edt_msleep(100) ;
...
dma_read_test(EdtDev *edt_uut,  EdtDev *edt_td, int *haltflag)
{
	...
	edt_intfc_write(edt_td, PCDT_CMD, 0 | use_pcd_src) ;
	...
	edt_intfc_write(edt_td, PCDT_CMD, PCDTEST_ENABLE | use_pcd_src ) ;
...
dma_write_test(EdtDev *edt_uut, EdtDev *edt_td)
{
    ...
    edt_intfc_write(edt_td, PCDT_CMD, PCDTEST_SETDNR | use_pcd_src) ;
    edt_intfc_write(edt_td, PCDT_CMD, PCDTEST_ENCHK | PCDTEST_SETDNR | use_pcd_src) ;
    /*
    ...
    */
    ...
    edt_intfc_write(edt_td, PCDT_CMD, PCDTEST_ENCHK | PCDTEST_SETDNR | PCDTEST_ENCMP | use_pcd_src) ;
    edt_msleep(200);

    /* start data from pcd */

    edt_intfc_write(edt_td, PCDT_CMD, PCDTEST_ENCHK | PCDTEST_ENCMP | use_pcd_src) ;
