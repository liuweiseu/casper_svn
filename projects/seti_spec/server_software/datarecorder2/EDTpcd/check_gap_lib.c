	...
	while (argc > 1 && argv[1][0] == '-') {
		switch(argv[1][1]) {
			...
			case 's': /*specified buffer size to use*/
				++argv;
				--argc; //BUG: check that user gave enough args.
			...
			case '-':
				...
				if (strcmp(argv[1], "--help") == 0) {
						// help message needs to be displayed, or something.
						// maybe return a value NEED_HELP and let caller do it.
						//usage();
						//exit(0);
						chkgap_usage();
						exit(0);
						...
						//user says board is PSS4_ID (for real!).
						d->ss4_flag = 1;
						d->ss1_flag = 0;
				}
				break;
...
void chkgap_initialize (CheckGapData *d) 
{
	...
	switch (d->exact_board_type) {
		...
		case PSS16_ID:
			...
			if (d->exact_board_type == PSS16_ID) {
					...
					//max_channels = 16 - is that true?, check w/ tyler. TODO
					//d->max_channels = 4;
					d->max_channels = 16;
			} else if (d->ss1_flag) {
					bitfilename = "pciss1test.bit";
		...
		default: /*user has unsupported board type*/
			fprintf(stderr, "Unsupported board type devid=0x%x (only PCDA_ID, PDVCL_ID, PSS4_ID, PSS16_ID, PGS4_ID, and PGS16_ID supported)\n", d->exact_board_type);
			exit(1);
			break;
	}
	...
	for (channel = 0; channel < d->nchannels; ++channel) {
		...
		if (edt_configure_ring_buffers(d->edt_p[channel], d->bufsize, d->loops, EDT_READ, NULL) == -1) {
...
void chkgap_run_test (CheckGapData *d, ResultHandler handler)
{
	...
	/*  
	if (d->board_type == BOARD_TYPE_PCD) {
	// reset the fifo in PCI Xilinx and in Interface Xilinx 
	// then we will start all the DMAs and then start the fifos 
		//The following two lines of code seem to make this program hang at
		//edt_wait_for_buffers. I don't know why.
		//Actually, it seems the following is essentially what happens
		//when flush_fifo is called, so I don't know why it was 
		//necessary to have this super low level stuff in here. 
		edt_reg_and(d->edt_p[0], PCD_CMD, ~PCD_ENABLE); 
		edt_reg_and(d->edt_p[0], EDT_DMA_INTCFG, ~EDT_RFIFO_ENB);
		
		//edt_reg_or(d->edt_p[0], EDT_DMA_INTCFG, EDT_RFIFO_ENB);
		//edt_reg_or(d->edt_p[0], PCD_CMD, PCD_ENABLE);
	}
	*/
	...
	/* 
	if (d->board_type == BOARD_TYPE_PCD) {
		// This apparently starts the fifos, but I think flush_fifo 
	...
	*/
	...
	for (i = 0; i < d->loops; ++i) {
#ifdef DEBUG
		printf("running: wait_4_bufs(edt_p[def_chan=%d], 1)\n", 
						d->default_channel);
#endif
