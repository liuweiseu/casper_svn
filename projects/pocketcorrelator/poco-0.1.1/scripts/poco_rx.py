#!/usr/bin/python
"""
Record data to Miriad files from the Pocket Correlator.
Author: Griffin Foster

Revisions:
2009-09-28: Generalized script for X inputs, Y channels
...
class C2M_RX:
    ...
    def __init__(self, loc, ants, n_chans=2048, n_ants=8, acc_len=8192, 
        ...
        eq_blocks=1,zero_ground=0,auto_offsets='',cross_real_offsets='',cross_imag_offsets=''):
        ...
        self.auto_offsets = auto_offsets
        self.cross_real_offsets = cross_real_offsets
        self.cross_imag_offsets = cross_imag_offsets
    ...
    def _process_packets(self):
        ...
        baseline_number = 0
        b0_data = numpy.zeros([self.n_chans])
        b1_data = numpy.zeros([self.n_chans])
        ...
        try:
            while not self._quit:
                ...
                if not p == None:
                    ...
                    combine_data = numpy.zeros(len(p['data'])/4)
                    if p['offset'] in [0,1,2,3,4,5,6,7]:
                        combine_data = self._auto_parse(p['data'])
                    else:
                        combine_data = self._cross_parse(p['data'])
                    baseline_slice = p['offset'] % self.pkts_per_bram
                    b0_data[(len(p['data'])/8)*baseline_slice:(len(p['data'])/8)*(baseline_slice+1)] = combine_data[::2]
                    b1_data[(len(p['data'])/8)*baseline_slice:(len(p['data'])/8)*(baseline_slice+1)] = combine_data[1::2]
                    if baseline_slice == self.pkts_per_bram - 1:
                        if p['offset'] in self.auto_offsets:
                            full_int_data[:,baseline_number,0] = b0_data
                            full_int_data[:,baseline_number,1] = numpy.zeros_like(b0_data)
                            baseline_number += 1
                            full_int_data[:,baseline_number,0] = b1_data
                            full_int_data[:,baseline_number,1] = numpy.zeros_like(b1_data)
                            baseline_number += 1
                        elif p['offset'] in self.cross_real_offsets:
                            full_int_data[:,baseline_number,0] = b0_data
                            full_int_data[:,baseline_number+1,0] = b1_data
                        elif p['offset'] in self.cross_imag_offsets:
                            full_int_data[:,baseline_number,1] = b0_data
                            full_int_data[:,baseline_number+1,1] = b1_data
                            baseline_number += 2
...
if __name__ == '__main__':
    ...
    AUTO_OFFSETS = []
    AUTO_OFFSETS.extend([int(coef) for coef in config['auto_offsets'].split(",")])
    CROSS_REAL_OFFSETS = []
    CROSS_REAL_OFFSETS.extend([int(coef) for coef in config['cross_real_offsets'].split(",")])
    CROSS_IMAG_OFFSETS = []
    CROSS_IMAG_OFFSETS.extend([int(coef) for coef in config['cross_imag_offsets'].split(",")])
    ...
    udp_rx = C2M_RX(loc=LOC, n_chans=N_CHANS, ants=ants, 
        ...
        auto_offsets=AUTO_OFFSETS,cross_real_offsets=CROSS_REAL_OFFSETS,
        cross_imag_offsets=CROSS_IMAG_OFFSETS)
