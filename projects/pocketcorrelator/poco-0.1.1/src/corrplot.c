/* History
 *
 * Version 1.0: Original corrplot code. (asiemion)
 * Version 1.1: added logfile to command line options (gfoster)
 *
 */
...
for (opt=1; opt<argc; opt++) {
	if(strcmp(argv[opt], "-h") == 0) {
		...
		printf("\t -l : log scale for amplitude plot, default: normal scale\n");
		exit(0);
...
if ((fp = fopen(logfile, "r")) == NULL) printf("\nError: Cannot open the file\n");
