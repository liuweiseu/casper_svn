...
class MiriadParams(dict):
    ...
    def __init__(self, **kwargs):
        ...
        ants = [aipy.phs.Antenna(a[0],a[1],a[2],beam) for a in prms['ants']]
        # Set antenna positions in miriad file
        #print '(c2m.py): Array location: ',prms['location']
        self.aa = aipy.phs.AntennaArray(ants=ants, location=prms['location'])
        n_ants = len(self.aa.ants)
        print '\tN Antennas: %i'%(n_ants)
        print '\tBandwidth: %f GHz'%(bandwidth)
        print '\tSDF: %f GHz'%(sdf)
        print '\tInt Time: %f s'%(int_time)
        print '\tArray location: ',prms['location']
...
class MiriadRecorder(MiriadParams):
    ...
    def conv_time(self, t):
        ...
        date."""
        return ephem.julian_date(time.strftime('%Y/%m/%d %H:%M:%S', time.gmtime(t)) + ('%1.6f' % (t % 1))[1:])
