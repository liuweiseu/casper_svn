...
Revisions:
2008-06-24	GSF added type to __init__ to handle poco packets correctly
				unpack_header() and convert_raw_data() updated to handle poco packet format
...
class CasperN_Packet:
    ...
    def __init__(self, endian='<', type='corr'):
        self.data_fmt = 'i'
        self.word_size = struct.calcsize(self.data_fmt)
        self.endian = endian
        self.type = type
        if self.type=='corr': self.header_fmt = '%sQLLLL' % (endian)		#24 byte header for correlator
        elif self.type=='poco': self.header_fmt = '%sQBIII' % (endian)	#21 byte header for poco
    ...
    def unpack_header(self, buf):
        ...
        if (buf != None) and (len(buf) >= self.header_size):
            ...
            #print ','.join([(hex(ord(m))+'0')[2:4] for m in buf[:self.header_size]])
            d = datetime.datetime.now()
            dm = d.microsecond/1000000.0
            ...
            ds1 = d +dm
            #print '%12.6f' % ds1
            #print decimal.Decimal(ds),
            #print type(decimal.Decimal(ds))
            #print float(decimal.Decimal(ds))
            #print 123456789.0 + dm
            #print time.mktime(d.timetuple()),d.microsecond/1000000.0
            #print time.time(),
            #if self.type == 'poco': return {'time':time.time(), 'xeng':v[1], 'offset':v[2], 'flags':v[3],'data_size':1024}
            if self.type == 'poco': return {'time':ds1, 'xeng':v[1], 'offset':v[2], 'flags':v[3],'data_size':1024}
            else: return {'time':v[0], 'xeng':v[1], 'offset':v[2], 'flags':v[3],'data_size':v[4]}
        else:
    ...
    def convert_raw_data(self, d):
        ...
        #if self.type == 'poco': fmt = '>%di' % (d['data_size'] / self.word_size)
        if self.type == 'poco':
            if d['offset'] in [0,1,2,3,12,13,14,15,32,33,34,35,52,53,54,55]:
                fmt = '>%dB' % (d['data_size'])
            else:
                #fmt = '>%db' % (d['data_size'])
                fmt = '>%dB' % (d['data_size'])
        else: fmt = '<%dI' % (d['data_size'] / self.word_size)
