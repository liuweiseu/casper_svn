...
#define NPACK SPECTRA/32
#define PORT 6969

#define VERSION "udprec 1.1 - 06.17.08"
...
int main(int argc, char **argv)
{
  ...
  for (c=1; c<argc; c++)
  {
	if(strcmp(argv[c], "-h") == 0)
	{
		...
		printf("\t -p [prefix] : write log files with prefix, default is poco_data\n");
		printf("\t -i [iterations] : number of files to produce, default is 1\n");
		printf("\t -n [packets] : number of spectra per iteration, default is 78125\n");
		exit(0);
