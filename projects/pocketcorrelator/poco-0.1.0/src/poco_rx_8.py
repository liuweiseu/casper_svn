...
class C2M_RX:
    ...
    def _process_packets(self):
        ...
        auto_offsets = [0,3,8,13]
        cross_real_offsets = [1,4,6,9,11,14,16,18,20,22,24,26,28,30]
        cross_imag_offsets = [2,5,7,10,12,15,17,19,21,23,25,27,29,31]
        current_offset = 0
        ...
        try:
            while not self._quit:
                ...
                if not p == None:
                    ...
                    if p['offset'] is 0:
                        combine_data = _auto_parse(p['data'][0:255])
                        ...
                        combine_data = _cross_parse(p['data'][768:1023])
                        full_int_data[:,baseline_number,0] = combine_data[::2]
                        full_int_data[:,baseline_number+1,0] = combine_data[1::2]
                    elif p['offset'] is 3:
                        combine_data = _cross_parse(p['data'][0:255])
                        full_int_data[:,baseline_number,1] = combine_data[::2]
                        full_int_data[:,baseline_number+1,1] = combine_data[1::2]
                        baseline_number += 2
                        
                        combine_data = _auto_parse(p['data'][256:511])
                        full_int_data[:,baseline_number,0] = combine_data[::2]
                        full_int_data[:,baseline_number,1] = numpy.zeros_like(combine_data[::2])
                        baseline_number += 1
                        full_int_data[:,baseline_number,0] = combine_data[1::2]
