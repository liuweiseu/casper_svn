...
class C2M_RX:
    ...
    def __init__(self, loc, ants, n_chans=2048, n_ants=8, acc_len=8192, 
        ...
        self.bl_order = poco.sim_cn_data.get_bl_order(self.n_ants)
    ...
    def _process_packets(self):
        ...
        auto_offsets = [3,15,35,55]
        cross_real_offsets = [7,19,27,39,47,59]
        cross_imag_offsets = [11,23,31,43,51,63]
        full_int_data = numpy.zeros([self.n_chans,self.n_bls,2])
        baseline_number = 0
        baseline_data = numpy.zeros([self.n_chans]) 
        print 'Expecting total dump size from each x eingine: %i bytes'%total_bytes_per_xeng

        #get a starting timestamp:
        p = pkt.unpack(correlator.get_a_packet())
        last_offset = p['offset']
        while last_offset < 63:
            p = pkt.unpack(correlator.get_a_packet())
            last_offset = p['offset']
        if p == None:
        ...
        try:
            while not self._quit:
                ...
                if not p == None:
                    ...
                    if p['offset'] in [0,1,2,3,12,13,14,15,32,33,34,35,52,53,54,55]:
                        for d in range(len(p['data'])/4):
                            d0 = p['data'][d*4]
                            d1 = p['data'][d*4+1]*256
                            d2 = p['data'][d*4+2]*65536
                            d3 = p['data'][d*4+3]*16777216
                            df = d0 + d1 + d2 + d3 
                            combine_data[d] = df
                    else:
                        for d in range(len(p['data'])/4):
                            if p['data'][d*4+3] > 127:
                                d0 = -1*(255-p['data'][d*4])
                                d1 = -1*(255-p['data'][d*4+1])*256
                                d2 = -1*(255-p['data'][d*4+2])*65536
                                d3 = -1*(255-p['data'][d*4+3])*8388608
                            else:
                                d0 = p['data'][d*4]
                                d1 = p['data'][d*4+1]*256
                                d2 = p['data'][d*4+2]*65536
                                d3 = p['data'][d*4+3]*8388608
                            df = d0 + d1 + d2 + d3 
                            combine_data[d] = df
                    baseline_slice = p['offset'] % 4
                    baseline_data[(len(p['data'])/4)*baseline_slice:(len(p['data'])/4)*(baseline_slice+1)] = combine_data
                    if baseline_slice == 3:
                        #baseline_data = baseline_data/64
                        baseline_data_flip = numpy.zeros_like(baseline_data)
                        baseline_data_flip[0:self.n_chans/2-1] = baseline_data[self.n_chans/2:self.n_chans-1]
                        baseline_data_flip[self.n_chans/2:self.n_chans-1] = baseline_data[0:self.n_chans/2-1]
                        baseline_data = baseline_data_flip
                        if p['offset'] in auto_offsets:
                            full_int_data[:,baseline_number,0] = baseline_data
                            full_int_data[:,baseline_number,1] = numpy.zeros_like(baseline_data)
                            #print min(baseline_data),max(baseline_data)
                            baseline_number += 1
                        elif p['offset'] in cross_real_offsets:
                            full_int_data[:,baseline_number,0] = baseline_data
                            #print min(baseline_data),max(baseline_data)
                            #print p['data'][100], p['data'][101],p['data'][102],p['data'][103]
                        elif p['offset'] in cross_imag_offsets:
                            full_int_data[:,baseline_number,1] = baseline_data
                            #print min(baseline_data),max(baseline_data)
                            #print p['data'][100], p['data'][101],p['data'][102],p['data'][103]
                            baseline_number += 1
                    #print time.time()-debug_time
                    #Check if we need to save this integration:
                    if (p['offset']+1) == n_integrations * pieces_per_int:
                        timestamp = p['time']
...
if __name__ == '__main__':
    ...
    TX_PKT_HEADER = config['tx_pkt_header']
    ...
    udp_rx = C2M_RX(loc=LOC, n_chans=N_CHANS, ants=ants, 
        ...
        pkts_per_bram=PKTS_PER_BRAM,brams_per_pkt=BRAMS_PER_PKT,nbrams=NBRAMS,design_name=DESIGN_NAME)
