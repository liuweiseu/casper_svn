...
static pid_t start_tap(struct abs_state *as, char *bd, char *mac, char *ip, char *port)
{
  pid_t pid;
  int fd, fi;
#define BUFFER 256
...
#undef FD_RANGE
  ...
  snprintf(buffer, BUFFER - 1, "%s/%s", as->s_process, bd);
  ...
  execlp("tgtap", "tgtap", "-b", buffer, "-a", ip, "-m", mac, "-p", port, NULL);
...
static int insert_file(struct abs_state *s, char *name)
{
  ...
  if(!access(ptr, R_OK | W_OK)) {
    /* R/W access OK */
    fd = open(ptr, O_RDWR);
  } else if(!access(ptr, R_OK)) {
    /* R/O access OK */
...
static void check_exit(struct katcp_dispatch *d)
{
  struct abs_state *s;
  pid_t collect;
  int status;
...
int tap_stop_cmd(struct katcp_dispatch *d, int argc)
{
  ...
  as = get_state_katcp(d);

  if(as->s_tap <= 0){
    log_message_katcp(d, KATCP_LEVEL_WARN, SUBSYSTEM, "no tap process running");
    return KATCP_RESULT_FAIL;
  }

  kill(as->s_tap, SIGKILL);
  as->s_tap = (-1);
...
int tap_start_cmd(struct katcp_dispatch *d, int argc)
{
  ...
  if(argc <= 2){
    log_message_katcp(d, KATCP_LEVEL_ERROR, SUBSYSTEM, "need a borph register and an ip as parameters");
    return KATCP_RESULT_FAIL;
  }

  bd = arg_string_katcp(d, 1);
  mac = arg_string_katcp(d, 2);
  ip = arg_string_katcp(d, 3);
  port = arg_string_katcp(d, 4);

  if((bd == NULL) || (ip == NULL)){
    log_message_katcp(d, KATCP_LEVEL_ERROR, SUBSYSTEM, "unable to acquire parameters");
    return KATCP_RESULT_FAIL;
  }

  if(as->s_pid <= 0){
    log_message_katcp(d, KATCP_LEVEL_WARN, SUBSYSTEM, "no bof file programmed");
  }

  if(as->s_tap > 0){
    log_message_katcp(d, KATCP_LEVEL_ERROR, SUBSYSTEM, "terminating existing tap instace");
    kill(as->s_tap, SIGKILL);
    as->s_tap = (-1);
  }

  as->s_tap = start_tap(as, bd, mac, ip, port);
  if(as->s_tap <= 0){
    log_message_katcp(d, KATCP_LEVEL_ERROR, SUBSYSTEM, "unable to start tap process");
    return KATCP_RESULT_FAIL;
  }

  log_message_katcp(d, KATCP_LEVEL_DEBUG, SUBSYSTEM, "started tap process as pid %d", as->s_tap);
...
int progdev_cmd(struct katcp_dispatch *d, int argc)
{
  ...
  if(s->s_tap > 0){
    log_message_katcp(d, KATCP_LEVEL_WARN, SUBSYSTEM, "killing tap process");
    kill(s->s_tap, SIGKILL);
    s->s_tap = (-1);
  }
...
int do_startup(struct katcp_dispatch *d, struct abs_state *s)
{
  ...
  result += register_katcp(d, "?progdev",  "programs an image (?progdev [boffile])", &progdev_cmd);
  ...
  result += register_katcp(d, "?read",     "reads arg3 bytes starting at arg2 offset from register arg1 (?read register-name byte-offset byte-count)", &read_cmd);
  ...
  result += register_katcp(d, "?write",     "writes arbitrary data lengths to a named register (?write register-name byte-offset data)", &write_cmd);
  ...
  result += register_katcp(d, "?echotest", "basic network echo tester (?echotest ip-address ip-port byte-count)", &echotest_cmd);

  result += register_katcp(d, "?tap-start","start the tap server (?tap-start 10gbe-register-name mac-address ip-address fabric-port)", &tap_start_cmd);
  result += register_katcp(d, "?tap-stop", "stop the tap server (?tap-stop)", &tap_stop_cmd);
