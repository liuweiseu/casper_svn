...
static unsigned int dops2cycles(unsigned long speed, unsigned int ps)
{
        ...
        psc = 1000000000 / (speed / 1000);

        return (ps / psc) + ((ps % psc) ? 1 : 0);
}
#else
...
{
	unsigned char data[2];
	unsigned int chip, v, t, i, j, x, sz, ms, rr, rf, st;
	unsigned int *ptr;
	unsigned long speed;
	...
	/* could use a less agressive mode by quitting for a lower x and more for a higehr x */
	for(i = 7, x = 0; (i >= 2) && (x < 2); i--){
		if(tmp & (0x1 << i)){
...
#define MAX_SUPPORTED_RAM 2048 /* can only map 2G, kernel needs work to see more than 1G */
	...
	for(i = 0; i < sz; i++){
		ptr = (unsigned int *)(i * 1024 * 1024);
#ifdef DEBUG
		printf("dram: clearing %dM\n", i);
#endif
