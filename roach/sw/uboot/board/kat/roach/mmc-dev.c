                ...
                case MMC_SEND_OP_COND:
                    ...
                    if(!(send_command(&cmd1, MMC_SLOW_CLOCK))){
                        ...
                        if(!(get_response(&info))){
                            ...
                            printf("error: Response Failure\n");
                            state = MMC_SEND_OP_COND;
                            return -1;
                        }
                    }
                    else{
                        printf("error: Sending Error:MMC_SEND_OP_COND\n");
                        state = MMC_SEND_OP_COND;
                        return -1;
                    }
                    break;
                case MMC_ALL_SEND_CID:
                    ...
                    if(!(send_command(&cmd2, MMC_FAST_CLOCK))){
                        ...
                        if(!(get_response(&info))){
                            ...
                            printf("error: No response received in MMC_ALL_SEND_CID state\n");
                        ...
                        state = MMC_GO_IDLE_STATE;
                        printf("error: Send Error:MMC_ALL_SEND_CID STATE\n");
                        return -1;
                    }
                    break;
