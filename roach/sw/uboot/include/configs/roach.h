...
#define CONFIG_LAST_STAGE_INIT	1	/* Call misc_init_r		*/
 ...
 */
#define CONFIG_CMD_ROACHMMC			/* undef to save memory		*/
...
#define CONFIG_BOOTDELAY	10	/* autoboot disabled		*/
...
/* content of eeprom : ENG=1, SRC=0, (don't care, PLLOUTA==PLLOUTB) LFBDV=3, SEL=0, TUNE=1110111100, FBDV=5, FBDVA=2, FBDVB=2, PRADV0=1, PRBDV0=3, OPBDV0=2 */
...
#define	CONFIG_EXTRA_ENV_SETTINGS					\
	...
	"init_eeprom=echo Programming EEPROM for configuration H;" \
		"imw 0x52 0.1 87; imw 0x52 1.1 78; imw 0x52 2.1 52; imw 0x52 3.1 4e;" \
		"imw 0x52 4.1 0d; imw 0x52 5.1 57; imw 0x52 6.1 a0; imw 0x52 7.1 30;" \
		...
		"rmon w 0xffff 2; rmon w 0x3c3 1\0" \
	"bit=report clear;" \
		...
		"cp.b 0xfc000000 0x200000 20000; check flash-mem-cmp cmp.b 0x200000 0xfc000000 0x20000;" \
		"check flash-erase erase 0xfc000000 +20000;" \
		"cp.b 0x200000 0xfc000000 0x20000; check flash-write cmp.b 0x200000 0xfc000000 0x20000;" \
		"usb start;" \
		"check usb-load fatload usb 0 0x200000 roach_bsp.bin;" \
		"check selectmap-program roachsmap 0x200000;" \
		"setenv ethaddr 02:02:02:02:02:02;" \
		"setenv netretry no;" \
		"check net-dhcp dhcp;" \
		"check fpga-dram-test roachddr;" \
		"check qdr-0-test roachqdr 0;" \
		"check qdr-1-test roachqdr 1;" \
		"check zdok-0-test roachzdok 0;" \
		"check zdok-1-test roachzdok 1;" \
	...
	"kernel_addr=0xfc000000\0"  \
	"partitions=physmap-flash.0:1792k(linux),256k@0x1c0000(fdt),8192k@0x200000(root),54656k@0xa00000(usr),256k@0x3f60000(env),384k@0x3fa0000(uboot)" \
	"fdt_addr=0xfc1c0000\0" \
	"boot=bootm ${kernel_addr}\0" \
	"bootargs=console=ttyS0,115200 mtdparts=physmap-flash.0:1792k(linux),256k@0x1c0000(fdt),8192k@0x200000(root),54656k@0xa00000(usr),256k@0x3f60000(env),384k@0x3fa0000(uboot) root=/dev/mtdblock2\0" \
	"yget=loady 0x200000\0" \
	"update=protect off 0xfffa0000 0xffffffff;"	\
		"era 0xfffa0000 0xffffffff;"	\
		"cp.b 0x200000 0xfffa0000 ${filesize};" \
		"protect on 0xfffa0000 0xffffffff\0" \
  	"soloboot=setenv bootargs console=ttyS0,115200 mtdparts=${partitions} root=/dev/mtdblock2; bootm 0xfc000000\0" \
  	"mmcboot=setenv bootargs console=ttyS0,115200 mtdparts=${partitions} root=b301 rootdelay=1; bootm 0xfc000000\0" \
	"usbboot=setenv bootargs console=ttyS0,115200 mtdparts=${partitions} rootdelay=8 root=/dev/sda1; bootm 0xfc000000\0" \
        "netboot=dhcp 0x400000; setenv bootargs console=ttyS0,115200 mtdparts=${partitions} root=${rootpath} ip=dhcp; bootm 0x400000\0" \
	...
	"newroot=run yget; era 0xfc200000 0xfc9fffff; cp.b 0x200000 0xfc200000 ${filesize}\0" \
        "initboot=echo; echo type  run netboot  to boot via dhcp+tftp+nfs; echo type  run soloboot  to run from flash without network; echo type  run mmcboot to boot using filesystem on mmc/sdcard;  echo type  run usbboot  to boot using filesystem on usb; echo type  run bit  to run tests; echo\0" \
	"clearenv=erase 0xfff60000 0xfff9ffff\0" \
	""
