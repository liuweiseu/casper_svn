...
static int start_tap(struct katcp_dispatch *d, char *tapdevice, char *name, char *ip, char *port, char *mac)
{
  pid_t pid;
  int fd, fi, result, i;
  char *vector[12];
...
#ifdef DEBUG
  fprintf(stderr, "tap: stopping any existing instance of %s\n", name);
#endif
  ...
  if(pid != 0){
    ...
    if(watch_type_shared_katcp(d, name, pid, RAW_TYPE_TGTAP, &current_tgtap_watch) < 0){
    ...
    return 0;
    /* done at this point, child continues */
  }
...
{
  char *name, *tapdevice, *ip, *port, *mac;

  mac = NULL;
  port = NULL;

  if(argc <= 4){
    log_message_katcp(d, KATCP_LEVEL_ERROR, NULL, "need at least a tap device name, register name, a mac address and an ip address");
    return KATCP_RESULT_INVALID;
  }

  tapdevice = arg_string_katcp(d,1);
  name = arg_string_katcp(d, 2);
  ip = arg_string_katcp(d, 3);

  if((tapdevice == NULL) || (ip == NULL) || (name == NULL)){
  ...
  if(start_tap(d, tapdevice, name, ip, port, mac) < 0){
    return KATCP_RESULT_FAIL;
  }
