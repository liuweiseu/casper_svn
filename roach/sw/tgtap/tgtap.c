...
/*  Issues ARP requests constantly, every ~100ms */

/* linux kernels can do arp gleaning, might be useful to enable here */

/* Author: Marc Welz
 * Modified: Jason Manley, 2009-07-02 */
...
{
  /* TODO: NOT WORKING. could be done using pwrite too */
...
int receive_borph(struct getap_state *gs)
{
  ...
  if(lseek(gs->s_bfd, GO_BUFFER_SIZES, SEEK_SET) != GO_BUFFER_SIZES){
    fprintf(stderr, "rxb: unable to seek to 0x%x\n (RX buffer size)", GO_BUFFER_SIZES);
    return -1;
  }
  if(read(gs->s_bfd, &buffer_sizes, 4) != 4){
  ...
  if(len <= 0){
#ifdef DEBUG
    fprintf(stderr,"Nothing to read: 0x%x\n",buffer_sizes);
#endif
    return 0;
  }
...
static int send_borph(struct getap_state *gs, unsigned char *data, unsigned int len)
{
  ...
  if(gs->s_verbose > 1){
    fprintf(stderr, "txb: sending packet of %u bytes\n", len);
  }
...
#ifdef DEBUG
      fprintf(stderr, "txb: expanded frame of %u to %u bytes\n",len,MIN_FRAME);
#endif
      len = MIN_FRAME;
    }
  }
  ...
  buffer_sizes = ((len + 7) / 8)<<16;
#ifdef DEBUG
    fprintf(stderr, "txb: sending %u words\n",buffer_sizes>>16);
#endif
  if(write(gs->s_bfd, &buffer_sizes, 4) != 4){
...
int setup_borph(struct getap_state *gs)
{
  ...
  if(gs->s_port){
    ...
    v = (0xff<<16) + (0xff<<16) + (gs->s_port);
    if(write_u32_borph(gs, GO_EN_RST_PORT, v)){
      return -1;
    }
    /* Next, remove core from reset state: */
    v = (0x00<<16) + (0xff<<16) + (gs->s_port);
    if(write_u32_borph(gs, GO_EN_RST_PORT, v)){
        return -1;
    }
  }
...
{
  /* We need to have the RX checked much more often than sending out ARP requests */
  /*  Consider randomising delay between subsequent checks to prevent this client only recieving arp requests from certain hosts. */
  int result, max, busy, arp_period, arp_index, rx_index;
  ...
  rx_index = 1;
  srand(time(0));
  arp_period = ARP_PERIOD + (rand()&(0xf));
  fprintf(stdout,"ARP period set to %i usec\n",arp_period*POLL_INTERVAL);
  announce_arp(gs);

  while(run){
    rx_index++;
    /* send out arp packets. loop through all 254 addresses.  */
    /* send an arp packet every ARP_PERIOD main loop iterations. */
    if (rx_index >= ARP_PERIOD){
      if(arp_index < 255){
        if(busy == 0){
          rx_index = 0;
          request_arp(gs, arp_index);
          if(arp_index>=254){
            arp_index=1;
          }
          else{
            arp_index++;
          } /* end if arp batch not finished */
        } /* end if not busy */
      } /* end if arp index < 255 */
    } /* end if rx index >= arp period */
